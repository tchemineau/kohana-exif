...
if ($result['ValidJpeg'] == 1) {
	...
	if (!isset($result['SubIFD']['FocalLengthIn35mmFilm'])) {
        ...
        $result['SubIFD']['CCDWidth'] = getCCDWidth($result);
...
function getCCDWidth(&$result, $precision = 0) {
        $width = (isset($result['SubIFD']['ExifImageWidth']))
                    ? $result['SubIFD']['ExifImageWidth']
                    : 0;
        
        $units = (isset($result['SubIFD']['FocalPlaneResolutionUnit']))
                    ? $result['SubIFD']['FocalPlaneResolutionUnit']
                    : '';
	...
	switch ($units) {
        ...
        $xres = (isset($result['SubIFD']['FocalPlaneXResolution']))
                    ?  $result['SubIFD']['FocalPlaneXResolution']
                    : '';

	if (($width != 0) && !empty($units) && !empty($xres) && !empty($width)) {
		$ccdwidth = (intval($width) * $unitfactor) / $xres;
		return round($ccdwidth, $precision) . ' mm';
	}
	return null;
}
